...
public class Picture extends SimplePicture {
	...
	*/
	public void grayScale() {
		Pixel[][] pixels = this.getPixels2D(); // retrieves the 

		// Pixel[][] pixels = this.getPixels2D();     
		Pixel pixel = null; // placeholder 
		int avg = 0;
		for (int row = 0; row < pixels.length; row++) { // loops through all rows
			for (int col = 0; col < pixels[0].length; col++) { // loops through al columns
								pixel = pixels[row][col]; // gets the pixel at the coordinate       
								avg = (int)Math.round(pixel.getAverage());
								pixel.setRed(avg);
								pixel.setGreen(avg);
								pixel.setBlue(avg);
			}     
  ...
  */
	...
	public void keepOnlyBlue() {     
		...
		// Pixel[][] pixels = this.getPixels2D();     
		Pixel pixel = null; // placeholder variable
		for (int row = 0; row < pixels.length; row++) { // loops through all rows
			for (int col = 0; col < pixels[0].length; col++) { // loops through al columns
								pixel = pixels[row][col]; // gets the pixel at the coordinate       
		 						pixel.setRed(0); // sets the red value to zero
		 						pixel.setGreen(0); // sets the green value to zero
			}     
		}  
	}
	public void keepOnlyRed() {     
	   ...
	   for (int row = 0; row < pixels.length; row++) { // loops through all rows
		   for (int col = 0; col < pixels[0].length; col++) { // loops through al columns
							   pixel = pixels[row][col]; // gets the pixel at the coordinate       
								pixel.setBlue(0); // sets the red value to zero
								pixel.setGreen(0); // sets the green value to zero
		   }     
	   }  
   }
   ...
   for (int row = 0; row < pixels.length; row++) { // loops through all rows
	   for (int col = 0; col < pixels[0].length; col++) { // loops through al columns
						   pixel = pixels[row][col]; // gets the pixel at the coordinate       
							pixel.setRed(0); // sets the red value to zero
							pixel.setBlue(0); // sets the green value to zero
	   }     
   }  
}
	...
	public void edgeDetection(int edgeDist){
		...
		double dist = 0;
		for (int row = 0; row < pixels.length; row++) { // loops through all rows
			for (int col = 1; col < pixels[0].length; col++) { // loops through al columns
				leftPixel = pixels[row][col-1];
				rightPixel = pixels[row][col];
				dist = rightPixel.colorDistance(leftPixel.getColor());
				if (dist > edgeDist) {
