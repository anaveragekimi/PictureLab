...
public class PictureTester {
	...
	private static void testKeepOnlyBlue() {
    	...
    	sBeach.write("images/sBeach.jpg");
	...
	private static void testKeepOnlyGreen() {
		Picture scenic = new Picture("images/scenic.jpg");// You would change the var name and file name to your picture when you are testing your image.
    	Picture sScenic = scenic.scale(0.5,0.5);// scales the size down if you need to resize.
    	sScenic.write("images/sScenic.jpg");
  
		sScenic.explore();// shows the picture in a window
		sScenic.keepOnlyGreen();// Calls your keep only blue method
		sScenic.explore();//shows a new picture with only blue hues
	}

	private static void testKeepOnlyRed() {	
		Picture selfie = new Picture("images/selfie.jpg");// You would change the var name and file name to your picture when you are testing your image.
		Picture sSelfie = selfie.scale(0.5,0.5);// scales the size down if you need to resize.
		sSelfie.write("images/sScenic.jpg");

		sSelfie.explore();// shows the picture in a window
		sSelfie.keepOnlyRed();// Calls your keep only blue method
		sSelfie.explore();//shows a new picture with only blue hues
	}
	...
	private static void testNegate() {
		Picture swan = new Picture("images/scenic.jpg");
	...
	private static void testClearBlueOverValue(int i) {
		Picture picture = new Picture("images/blueMotorcycle.jpg");
	...
	private static void testGetCountRedOverValue(int i) {
		...
		Pixel[][] pixels = flower.getPixels2D();
		Pixel pixel = null;
		int count = 0;
		for (int row = 0; row < pixels.length; row++) {
			for (int col = 0; col < pixels[0].length; col++) {
	...
	private static SimplePicture decode(Picture moto) {
		...
		for (int row = 0; row < pixels.length; row++) {
			for (int col = 0; col < pixels[0].length; col++) {
								...
								} else {pixel.setColor(Color.white);}
			}     
		} 
		return pic;
	}
	...
	private static void hide(Picture pic, Picture msg) {
		...
		for (int row = 0; row < img.length; row++) {
			for (int col = 0; col < img[0].length; col++) {
								i = img[row][col];
								m = msgp[row][col];
								if (m.colorDistance(Color.black) < 100) {
									i.setRed(i.getRed()+1);
								}
			}     
		} 
